...
- [ ] **P0.1** Fix `venue_id=0` en analítica de búsqueda (`queue.py:38`)
- [ ] **P0.2** Fix `db.lastrowid` (`admin.py:581`)
- [ ] **P0.3** Fix borrado de venue / FKs `ON DELETE CASCADE`
- [ ] **P0.4** URL-encode del query de búsqueda (`youtube_search.py:20`)
- [ ] **P0.5** Transiciones a `playing` bajo `_playback_lock`
- [ ] **P0.6** Rate limit re-verificado dentro del lock
- [ ] **P1.1** Backups automáticos (cron `VACUUM INTO` o Litestream)
- [ ] **P1.2** Tarea de fondo horaria (limpieza + sesiones + poda de eventos)
- [ ] **P1.3** `bcrypt` → `asyncio.to_thread`
- [ ] **P1.4** Transacciones explícitas donde importan + migraciones atómicas
- [ ] **P1.5** Caché + auth en `/api/queue/search`
- [ ] **P1.6** nginx `worker_connections`
- [ ] **P1.7** Validar JWT en el handshake del WebSocket
