...
|---|---|---|---|
| De mesa (`useAdminDashboard.js:110-117`) | `/{slug}/registro` | `/{slug}/r` | `/{slug}/registro?utm_source=mesa&utm_medium=qr` |
| De pantalla (`Kiosk.vue:18-19`) | `/{slug}/registro` | `/{slug}/p` | `/{slug}/registro?utm_source=pantalla&utm_medium=qr` |
...
El texto que se imprime debajo del QR pasa a ser también `/{slug}/r`: más corto
...
- **No funciona en `npm run dev`.** El redirect vive en nginx, no en Vite: en
  local `/{slug}/r` da 404. Es aceptable — se verifica en `staging`.
...
- [x] El nginx responde **302** en `/{slug}/r` y `/{slug}/p`, con `Location`
...
- [ ] Repetido contra `https://st.repitela.com/{slug}/r`, con `Location` en `https://`.
- [ ] En GA4 el tráfico aparece agrupado bajo `medium=qr` y separable por
      `source` en `mesa` / `pantalla`, junto al `sticker` de la landing.
- [x] El QR no gana módulos: `/{slug}/r` es más corto que `/{slug}/registro`.
- [x] Un slug inexistente en `/{slug}/r` no rompe: redirige y la SPA muestra lo
