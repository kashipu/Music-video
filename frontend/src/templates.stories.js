...
// "negativo" es la variante para fondo oscuro; asi la elige AuthLoginForm.vue.
...
const loginTemplate = `
  <AuthSplitLayout :style="compact ? 'min-height:auto;padding:16px 12px;align-items:flex-start' : ''">
    <AuthLoginForm v-model:username="username" v-model:password="password" title="Admin La Terraza" subtitle="Ingresa para administrar tu bar" :logo-url="logoUrl" :show-google="!compact" @submit="submit">
...
function loginStory(compact = false) {
  return (_, { globals }) => ({
    ...
    setup: () => ({ compact, logoUrl: logoFor(globals.mode), username: ref('admin'), password: ref(''), submit: () => {} }),
    ...
    template: `
      <main style="min-height:100vh;padding:24px 16px;background:var(--bg);color:var(--text)">
        <div style="max-width:960px;margin:auto;display:grid;gap:16px">
          <header style="display:flex;align-items:center;justify-content:space-between;gap:12px"><div><h1 style="font-size:24px">La Terraza</h1><p style="color:var(--text-muted);margin-top:4px">Configuración del bar</p></div><Badge variant="success">Activo</Badge></header>
...
export const TodosLosTemas = {
  render: (_, { globals }) => ({
    components: { AuthSplitLayout, AuthLoginForm },
    setup: () => ({ themes: THEME_PRESETS, mode: globals.mode, logoUrl: logoFor(globals.mode), username: ref('admin'), password: ref(''), submit: () => {} }),
    template: `
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px">
          <section v-for="theme in themes" :key="theme.id" :data-venue-theme="theme.tokens" :data-theme="mode" style="overflow:hidden;border:1px solid var(--border);border-radius:var(--radius);background:var(--color-background);color:var(--color-text)">
            ...
            <AuthSplitLayout style="min-height:auto;padding:16px 12px;align-items:flex-start">
              <AuthLoginForm v-model:username="username" v-model:password="password" title="Admin La Terraza" subtitle="Ingresa para administrar tu bar" :logo-url="logoUrl" @submit="submit">
                <template #footer><a href="#">¿Olvidaste tu contraseña?</a></template>
              </AuthLoginForm>
            </AuthSplitLayout>
          </section>
